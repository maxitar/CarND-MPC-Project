    ...
    std::string s = hasData(sdata);
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double epsi = -std::atan(polydereval(coeffs, x));

          int latency = mpc.p_.latency; //milliseconds
          double dt_latency = mpc.p_.dt_latency;
          if (latency > 0) {
            double Lf = 2.67;
            x = v*dt_latency;
            y = 0.;
            psi = -v*delta*dt_latency/Lf;
            cte += v*std::sin(epsi)*dt_latency;
            epsi += -v*delta*dt_latency/Lf;
            v += a*dt_latency;
          }
